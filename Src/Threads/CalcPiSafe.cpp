///Builds:	g++ ~/Codes/Src/Threads/CalcPiSafe.cpp -lpthread -o ~/Codes/Output/PiSafe
///Runs:	~/Codes/Output/PiSafe
...
void *Thread_Sum(void *rank) {
	...
	for (i=my_first_i; i<my_last_i; i++, factor = -factor) {
	  pthread_mutex_lock(&m_sum);	 
		sum += factor/(2.0*i+1);//Controlled access to critical section
